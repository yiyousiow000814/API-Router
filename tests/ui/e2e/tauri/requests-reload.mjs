...
} from '../../support/runtime-utils.mjs'
import {
  runRequestsAnalyticsSwitchNoReloadCase,
  waitSectionHeading,
  waitVisible,
...
async function main() {
  ...
  try {
    ...
    await runRequestsAnalyticsSwitchNoReloadCase(driver, path.join(repoRoot, 'user-data', 'ui-artifacts', 'tauri', `requests-reload-${Date.now()}.png`))

    console.log('[ui:requests-reload] PASS')
  } finally {
    try {
...
main().catch((e) => {
  console.error(`[ui:requests-reload] FAIL: ${e?.stack || e}`)
  process.exitCode = 1
})
