...
function buildTwoProviderConfigWithOrder(): Config {
  return {
    listen: { host: '127.0.0.1', port: 4000 },
    routing: {
      preferred_provider: 'p1',
    ...
    provider_order: ['p2', 'p1'],
  }
}

function buildTwoProviderStatusWithoutWeeklyWindow(): Status {
  return {
    listen: { host: '127.0.0.1', port: 4000 },
    preferred_provider: 'p1',
    manual_override: null,
...
describe('findMissingBudgetHardCapToggleToDisable', () => {
  ...
  it('prioritizes provider_order when multiple providers have missing windows', () => {
    const result = findMissingBudgetHardCapToggleToDisable(
      buildTwoProviderConfigWithOrder(),
      buildTwoProviderStatusWithoutWeeklyWindow(),
    )
    expect(result).toEqual({ provider: 'p2', period: 'weekly' })
  })
})
