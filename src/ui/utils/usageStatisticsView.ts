...
export function buildUsageProviderFilterOptions(
...
export function buildUsageModelFilterOptions(
  usageCatalogModels: string[],
): string[] {
  return [...usageCatalogModels].sort((a, b) => a.localeCompare(b));
}

export type UsageModelFilterDisplayOption = {
  id: string;
  label: string;
  models: string[];
};

export function formatUsageModelDisplayName(
...
export function countUsageModelFilterDisplayOptionsSelected(
  options: UsageModelFilterDisplayOption[],
...
export function sanitizeSelectedFilterValues(
  selected: string[],
  options: string[],
): string[] {
...
export function buildUsageSharedCostView(usageByProvider: UsageProviderRow[]): {
    ...
    const keeper =
      [...rows].sort(
        (a, b) =>
          ...
          String(a.api_key_ref ?? "").localeCompare(
            String(b.api_key_ref ?? ""),
          ) ||
  ...
  usageByProvider.forEach((row) => {
    ...
    const daily = zeroed
      ? 0
      : row.estimated_daily_cost_usd != null &&
          Number.isFinite(row.estimated_daily_cost_usd)
        ? Number(row.estimated_daily_cost_usd)
        : null;
    const total = zeroed
      ? 0
      : row.total_used_cost_usd != null &&
          Number.isFinite(row.total_used_cost_usd)
...
export function buildUsageProviderDisplayGroups(
  ...
  return Array.from(groups.values()).map((group) => {
    const requests = group.rows.reduce(
    ...
    const effectiveDailyValues = group.rows
      .map((row) =>
        usageSharedCostView.effectiveDailyByRowKey.get(
    ...
    const effectiveTotalValues = group.rows
      .map((row) =>
        usageSharedCostView.effectiveTotalByRowKey.get(
          usageProviderRowKey(row),
        ),
      )
      .filter(
    ...
    const pricingSources = Array.from(
      new Set(
        group.rows
          .map((row) => String(row.pricing_source ?? "").trim())
          .filter(Boolean),
      ),
    );
    ...
    return {
      ...
      estimatedAvgRequestCostUsd:
        effectiveTotal != null && requests > 0
...
export function computeUsageProviderTotalsAndAverages(
  ...
  const totalReq = usageByProvider.reduce(
    (sum, row) => sum + (row.requests ?? 0),
    0,
  );
  const totalTok = usageByProvider.reduce(
    (sum, row) => sum + (row.total_tokens ?? 0),
    0,
  );
  ...
  return {
    ...
    totalTokPerReq,
    avgUsdPerReq: mean(
      usageByProvider.map((row) => row.estimated_avg_request_cost_usd),
    ),
    avgUsdPerMillion: mean(
      usageByProvider.map((row) => row.usd_per_million_tokens),
    ),
    avgEstDaily: mean(
      usageByProvider
        .filter(
          (row) =>
            !usageSharedCostView.zeroRowKeys.has(usageProviderRowKey(row)),
        )
        .map((row) =>
          usageSharedCostView.effectiveDailyByRowKey.get(
            usageProviderRowKey(row),
...
export function buildUsagePricingGroups(
  ...
  const modePriority = (providerName: string) => {
    const mode = (config?.providers?.[providerName]?.manual_pricing_mode ??
      "none") as UsagePricingMode;
  ...
  return Array.from(groups.values()).map((providers) => {
    const primaryProvider =
      [...providers].sort(
        (a, b) => modePriority(b) - modePriority(a) || a.localeCompare(b),
      )[0] ?? providers[0];
    return {
      id: providers.join("|"),
      providers,
