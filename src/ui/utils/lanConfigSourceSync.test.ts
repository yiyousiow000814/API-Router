...
describe('lanConfigSourceSyncSignature', () => {
  ...
  it('changes when peer remote update status changes', () => {
    const base = {
      enabled: true as const,
      discovery_port: 38455,
      heartbeat_interval_ms: 2000,
    ...
    const accepted = lanConfigSourceSyncSignature({
      ...base,
      peers: [
        {
          ...base.peers[0],
    ...
    const running = lanConfigSourceSyncSignature({
      ...base,
      peers: [
        {
          ...base.peers[0],
    ...
    expect(accepted).not.toBe(running)
  })
})
