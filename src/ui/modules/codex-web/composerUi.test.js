...
- Clear stale pending UI.
  ...
  it("keeps every local branch visible after git meta loads", () => {
    const nodes = new Map();
    for (const id of [
      "mobileComposerRow",
      "mobilePromptWrap",
    ...
    const state = {
      ...
      activeThreadBranchOptions: [
        ...
        { name: "docs/repo-refactor-blueprint", prNumber: 168 },
      ],
      permissionPresetByWorkspace: {
        windows: "/permission full-access",
        wsl2: "/permission auto",
    ...
    const menuHtml = nodes.get("composerBranchPickerMenu")?.innerHTML || "";
    expect(menuHtml).toContain("main");
    expect(menuHtml).toContain("feat/codex-web-branch-picker");
    expect(menuHtml).toContain("chore/web-codex-terminal-communication");
    expect(menuHtml).toContain("docs/repo-refactor-blueprint");
  });
