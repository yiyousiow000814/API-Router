...
#[tauri::command]
pub(crate) fn get_config_toml(state: tauri::State<'_, app_state::AppState>) -> Result<String, String> {
    std::fs::read_to_string(&state.config_path).map_err(|e| e.to_string())
}

#[tauri::command]
pub(crate) fn set_config_toml(
    state: tauri::State<'_, app_state::AppState>,
    toml_text: String,
) -> Result<(), String> {
    let mut cfg: crate::orchestrator::config::AppConfig =
        toml::from_str(&toml_text).map_err(|e| format!("invalid config.toml: {e}"))?;

    // Keep the same migrations/invariants as startup loading.
    let mut changed = false;
    changed |= app_state::migrate_provider_name(&mut cfg, "provider_a", "provider_1");
    changed |= app_state::migrate_provider_name(&mut cfg, "provider_b", "provider_2");
    changed |= app_state::normalize_provider_order(&mut cfg);
    cfg.routing
        .session_preferred_providers
        .retain(|_, provider| cfg.providers.contains_key(provider));

    // Persist any inline provider api_key into secrets and clear from config.
    for (name, provider) in cfg.providers.iter_mut() {
        if !provider.api_key.is_empty()
            && provider.api_key != "REPLACE_ME"
            && state.secrets.set_provider_key(name, &provider.api_key).is_ok()
        {
    ...
    // Ensure preferred_provider always points to an existing provider.
    if !cfg.providers.contains_key(&cfg.routing.preferred_provider) {
        let fallback = cfg
            .provider_order
            .iter()
            .find(|name| cfg.providers.contains_key(*name))
            .cloned()
            .or_else(|| cfg.providers.keys().next().cloned())
            .ok_or_else(|| "config.providers cannot be empty".to_string())?;
        cfg.routing.preferred_provider = fallback;
        changed = true;
    }

    {
        let mut runtime_cfg = state.gateway.cfg.write();
        *runtime_cfg = cfg;
    }
    {
        let cfg = state.gateway.cfg.read();
        let mut mo = state.gateway.router.manual_override.write();
        if mo.as_ref().is_some_and(|name| !cfg.providers.contains_key(name)) {
            *mo = None;
        }
    }
    state
    ...
    state.gateway.store.add_event(
        "gateway",
        "info",
        "config.raw_toml_updated",
        if changed {
            "config.toml updated from raw editor (normalized)"
        } else {
            "config.toml updated from raw editor"
        },
        serde_json::Value::Null,
    );
    Ok(())
