...
fn usage_refresh_window_state(
) -> &'static std::sync::Mutex<UsageRefreshSummaryWindow> {
...
fn usage_refresh_on_success(now_ms: u64, providers: usize) -> (Option<UsageRefreshSummaryWindow>, Option<u64>) {
...
pub(crate) async fn refresh_quota(
    ...
    if snap.last_error.is_empty() && snap.updated_at_unix_ms > 0 {
        let message = "Usage refresh succeeded";
...
pub(crate) async fn refresh_quota_shared(
    ...
    let message = format!("Usage refresh succeeded (shared): {n} providers updated");
    state.gateway.store.add_event(
        "gateway",
        "info",
        "usage.refresh_succeeded",
